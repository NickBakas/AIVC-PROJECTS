...
for gen = 1:maxgen
    ...
    %% TRANSLATING CITY INDEXES TO DISTANCES
    %5 THIS WILL HELP DEFINE FITNESS
...
rank = zeros(size(Fitness));

for b=1:length(sortedFitness)
    rank = 1:length(sortedFitness);
    
    selectionProbability = rank / sum(rank);
end


%% ROULETTE RANGE CREATION
%range για δείκτη και αρίθμηση cumsum
    SummedProbability = cumsum(selectionProbability);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%     
    ...
    disp(off2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
    %% DISTANCES CALCUTION FOR THE OFFSPRING
    %9-10
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    %% INDEXING AND REPLACEMENT IN THE POPULATION (IF THE FITNESS EXCELS)
    
    [sortedFitness2,sortedIdx2] = sort(Fitness,'ascend');
    lowestFitness = sortedIdx2(1:2);
    
    if Fitness(sortedIdx2(1))<newOffspringFitness1
       Population(lowestFitness(1), :) = off1;
    end

    if Fitness(sortedIdx2(2))<newOffspringFitness2
...
figure;
plot(1:maxgen, bestFitnessPerGeneration, 'color',[1, 0.549, 0], 'LineWidth', 1);
hold on;
plot(1:maxgen, averageFitnessPerGeneration,'color',[0, 0, 0], 'LineWidth', 1);
plot(1:maxgen, worstFitnessPerGeneration, 'color',[0.3010 0.7450 0.9330], 'LineWidth', 1);
xlabel('Generations');
ylabel('Fitness');
...
figure;
plot(1:maxgen, bestTotalDistancesPerGeneration, 'color',[1, 0.549, 0], 'LineWidth', 1);
hold on;
plot(1:maxgen, averageTotalDistancesPerGeneration, 'color',[0, 0, 0], 'LineWidth', 1);
plot(1:maxgen, worstTotalDistancesPerGeneration, 'color',[0.3010 0.7450 0.9330],'LineWidth', 1);
xlabel('Generations');
ylabel('Distances');
